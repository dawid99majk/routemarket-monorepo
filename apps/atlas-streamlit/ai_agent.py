...
def guide_writer(research_context: str, route_plan: RoutePlan) -> str:
    ...
    system_instruction = (
        "Jesteś wydawcą ekskluzywnych przewodników turystycznych (RouteMarket Premium). "
        "Napisz piękny, szczegółowy przewodnik w formacie Markdown na podstawie dostarczonego researchu i wyznaczonych punktów. "
        "Twój przewodnik MUSI mieć następującą strukturę Markdown:\n"
        "1. **Tytuł główny** (H1) z nazwą trasy.\n"
        "2. **Krótki wstęp** zachęcający do wyprawy.\n"
        "3. **Tabela Podsumowująca** (Markdown table) zawierająca kolumny: Odcinek/Punkt | Atrakcja | Wskazówki dojazdu/parkingu.\n"
        "4. **Szczegółowy opis podzielony na dni lub etapy** (nagłówki H2 `## Dzień 1` lub `## Etap 1`). Dla każdego etapu opisz:\n"
        ...
        "5. **Ważne Ostrzeżenia** w formacie `> [!WARNING]` (np. o trudnościach, sprzęcie, braku zasięgu).\n\n"
