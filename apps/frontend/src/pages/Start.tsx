                ...
                <div className="flex gap-2 mt-5">
                  {must.slice(0, 6).map((p) => (
                    <div key={p.id} title={p.name}
                      ...
                      {p.image_url && <Zdjecie src={p.image_url} gdzie="kafelek" alt="" className="w-full h-full object-cover" />}
                    </div>
                  ))}
                </div>
              )}
                ...
                {podglad.map((m) => (
                  <button key={m.id} onClick={() => navigate(`/miejsce/${m.slug}`)}
                    ...
                    <div className="h-[92px] bg-muted">
                      {m.photos?.[0] && (
                        <Zdjecie src={m.photos[0]} gdzie="kafelek" alt="" className="w-full h-full object-cover" />
                      )}
                    </div>
                    <div className="p-2.5">
