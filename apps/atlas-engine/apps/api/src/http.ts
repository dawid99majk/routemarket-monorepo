...
import { geometryHandler } from "./routes/geometry.js";
import { researchHandler } from "./routes/research.js";

function createRoutes(): Route[] {
  return [
    route("POST", "/api/routes/geometry", geometryHandler),
    route("POST", "/api/routes/research", researchHandler),
    route("GET", "/", async ({ res }) => {
...
export async function readJson(req: IncomingMessage, maxBytes: number = 1_000_000): Promise<any> {
  const chunks: Buffer[] = [];
  let received = 0;
